...
import { arrayBufferToBase64Url } from "@/lib/utils";
...
export const authModule = define(() => {
  ...
  async function restoreSession(): Promise<boolean> {
    try {
      ...
      const storedCredentials = await storageService.getCredentialsForLogin();
  ...
  async function checkSupport(): Promise<AuthSupportInfo> {
    const support = await authService.checkSupport();
  ...
  > {
    ...
    try {
      ...
      if (result.prfOutput) {
        ...
        key = await cryptoService.deriveKeyFromPRF(result.prfOutput, {
          salt: result.prfSalt,
        });
      } else {
  ...
  > {
    ...
    try {
      ...
      const credentials = await storageService.getCredentialsForLogin();
      const allowCredentials = credentials.map((c) => c.credentialId);

      const result = await authService.authenticate({ allowCredentials });

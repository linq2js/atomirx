 ...
 * ```
 */
export type Atom<T, TFallback = undefined> = NoInfer<AtomState<T, TFallback>> & {
...
export type MutableAtom<T, TFallback = undefined> = Atom<T, TFallback> & {
     ...
     */
    set(value: Awaited<T> | PromiseLike<Awaited<T>> | ((prev: T) => Awaited<T>)): void;
    /**
 ...
 * ```
 */
export declare function readAll<const TGetters extends readonly Getter<any>[]>(getters: TGetters): {
 ...
 * ```
 */
export declare function readAll<TGetters extends Record<string, Getter<any>>>(getters: TGetters): {
    [K in keyof TGetters]: ReturnType<TGetters[K]>;
};
/**
